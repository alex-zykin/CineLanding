...
[Открыть демо ORBIT](https://cinelanding.alexey3484.chatgpt.site). ORBIT — вымышленный независимый кинотеатр, показанный как готовый клиентский лендинг, а не как презентация инструмента. На первом экране прокрутка страницы управляет 120 кадрами, извлеченными из проверенного перехода KIE. Русский язык включается в шапке, а выбор сохраняется в адресе страницы и в браузере.

Исходный код демо находится в каталоге [`site/`](site). Он показывает конечный результат процесса: семантический текст поверх постепенно загружаемой canvas-сцены, отдельное состояние для `prefers-reduced-motion` и полноценный адаптивный frontend. Только в финальном блоке раскрывается, что ORBIT вымышлен, и появляется ссылка на CineLanding.
